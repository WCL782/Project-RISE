// =========================================================================
// Project-RISE Self-Checking Testbench (2-Space Standard)
...
module tb_project_rise_core;
  ...
  // 實例化待測核心 (DUT) - 設定較短的縮影常數以加速 CI 模擬
  project_rise_core #(
  ...
  // 主測試程序
  initial begin
    // 波形錄製設定 (供 CI 產生 Artifact)
    $dumpfile("wave.vcd");
    $dumpvars(0, tb_project_rise_core);

    $display("=================================================");
    $display("   Project-RISE Hardware Verification Suite");
    $display("=================================================");
    ...
    pqc_ch2_data = 32'hDEAD_BEEF; // CH2 遭篡改/故障
    #20;
    ...
    check_signal("System Remains Powered",     pin_physical_power_off, 1'b0);

    // 恢復 TMR 正常
    pqc_ch2_data = 32'hA5A5_1234;
    #20;
    check_signal("Decoy Deactivated After Recovery", pin_decoy_trigger, 1'b0);

    // --- TEST 3: Zero-Heat Window & Thermal Anomaly ---
    $display("\n--- TEST 3: Thermal Anomaly & Window Count ---");
    thermal_sensor = 1; // 偵測到異物熱源
    #20;
    check_signal("Thermal Anomaly Power Lockdown", pin_physical_power_off, 1'b1);

    // 復位準備測試共識
    rst_n = 0; #20; rst_n = 1; thermal_sensor = 0; #20;

    // --- TEST 4: Decentralized Consensus Failure ---
    $display("\n--- TEST 4: Consensus Failure Test (1 Node Active) ---");
    node_status = 3'b001; // 低於過半共識 (只有 1 個 Node)
    #20;
    check_signal("Consensus Failure Power Off Trigger", pin_physical_power_off, 1'b1);

    // 復位
    rst_n = 0; #20; rst_n = 1; node_status = 3'b011; #20; // 2/3 通過

    // --- TEST 5: Mains Power Loss / HEMP Attack ---
    $display("\n--- TEST 5: Mains Power Loss (HEMP Simulation) ---");
    mains_power_ok = 0; // 斷電
    #20;
    check_signal("Power Cut Instant Lockdown", pin_physical_power_off, 1'b1);

    // --- TEST 6: RO-TRNG Output Check ---
    $display("\n--- TEST 6: RO-TRNG Random Entropy Output Check ---");
    #50;
    if (trng_entropy_out !== 32'h0 && trng_entropy_out !== 32'hFFFF_FFFF) begin
      $display("[PASS] TRNG Generated Entropy: 0x%h", trng_entropy_out);
      test_pass_count = test_pass_count + 1;
    end else begin
    ...
    if (test_fail_count > 0) begin
      ...
      $fatal(1); // 終止模擬並輸出錯誤碼 1，強制 CI 標示為紅燈
    end else begin
