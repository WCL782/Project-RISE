...
module project_rise_core #(
  ...
  always @(*) begin
    ...
    if (pqc_ch0_data == pqc_ch1_data) begin
      ...
      tmr_voted_data = pqc_ch0_data; // Fallback to CH0 under severe corruption
  ...
  wire [7:0] ro_out;
  (* keep = "true", S = "true" *) wire [7:0] ro_n;
  ...
  always @(*) begin
    ...
    case (current_state)
      ...
      ST_LOCKDOWN: begin
        next_state = ST_LOCKDOWN; // Irreversible Hard Lockdown
      end
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // Zeroize output bus during lockdown for zero data leak
